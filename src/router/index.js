...
const routes = [
  ...
  {
    name: "addressRegistrationMap",
    path: "/address-registration/map",
    component: Map,
...
const router = createRouter({
  ...
  routes,
  scrollBehavior() {
    //با تغییر صفحه اسکرول به ابتدای صفحه میرود
    return { top: 0 };
  },
});
router.beforeEach((to, from, next) => {
  if (to.meta.requiresAuth && !isAuthenticated.value) {
    //به صفحه  قبل از پر کردن اطلاعات در صفحه ورود به صفحه map را ندارد
